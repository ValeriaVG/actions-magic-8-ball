...
var run = async () => {
  ...
  if (!import_github.context.payload.comment || !import_github.context.payload.comment["body"]?.startsWith(prefix)) {
    return;
  }
  const issue_number = import_github.context.payload.pull_request?.number || import_github.context.payload.issue?.number;
  if (!issue_number) {
    return;
  }
