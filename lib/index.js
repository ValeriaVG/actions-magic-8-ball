...
function generateEightBallResponse() {
  const randomSize = import_crypto4.default.randomBytes(1)[0];
  const randomValues = import_crypto4.default.randomBytes(randomSize);
...
var run = async () => {
  var _a, _b, _c;
  const prefix = (0, import_core.getInput)("prefix");
  const github_token = (0, import_core.getInput)("GITHUB_TOKEN");
  if (!import_github.context.payload.comment || !((_a = import_github.context.payload.comment["body"]) == null ? void 0 : _a.startsWith(prefix))) {
    return;
  }
  const issue_number = ((_b = import_github.context.payload.pull_request) == null ? void 0 : _b.number) || ((_c = import_github.context.payload.issue) == null ? void 0 : _c.number);
  if (!issue_number) {
    return;
  }
  const octokit = (0, import_github.getOctokit)(github_token);
  const body = "> " + import_github.context.payload.comment["body"] + "\n\r@" + import_github.context.payload.comment["user"].login + " " + generateEightBallResponse();
